    ...
    <VWindow v-model="tab">
      ...
      <VWindowItem value="records">
        ...
        <VDataTable v-model="selected" :headers="recordHeaders" :items="records.items" item-value="history_id" :loading="loading" show-select class="desktop-table" :items-per-page="50" hide-default-footer>
          ...
          <template #item.query_title="{ item }"><div class="title-cell"><strong>{{ item.query_title || '未提取到中文片名' }}</strong><span>{{ item.query_year || '年份未知' }}</span></div></template>
...
.desktop-table { overflow:hidden; border:1px solid var(--line); border-radius:12px; background:rgb(var(--v-theme-surface)); }.desktop-table :deep(thead) { background:var(--soft); }.desktop-table :deep(.v-data-table__td),.desktop-table :deep(.v-data-table__th) { padding-inline:12px; }.title-cell { display:grid; gap:3px; padding-block:7px; }.title-cell span,.candidate-inline span,.reason { display:block; color:var(--muted); font-size:.76rem; }.candidate-inline { display:flex; align-items:center; gap:10px; padding-block:6px; }.candidate-inline>div:last-child { min-width:0; }.candidate-inline strong { display:block; }.reason { max-width:190px; margin-top:5px; line-height:1.35; }.row-actions { display:flex; gap:4px; }
